...
(defun secd-comp--comp (e n c)
  ...
  (if (atom e) (cons 'LD (cons e c))
    ...
    ;; 2 args
	    (if (eq (car e) 'cons)
		(secd-comp--comp (car (cdr e)) n (secd-comp--comp (car (cdr (cdr e))) n (cons 'CONS c)))
	      (if (eq (car e) 'eq)
...
(defun secd-compile-buffer (file)
  "Compiles source code in buffer into control list file `file.fasl'"
  (interactive "bSource buffer: ")
  (let* ((source (with-temp-buffer
		  (insert-buffer file)
		  (buffer-string)))
	 )
    (with-current-buffer (get-buffer-create "*SECD-COMPILED*")
      (erase-buffer)
      (insert
       (format "(setq bytecode (secd-comp--comp '%s nil '(STOP)))" source))
      (let ((ignore (eval-buffer)))
	(erase-buffer)
	(insert (format "%s" bytecode))
	)
